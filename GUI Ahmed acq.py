...
def main_app():
    ...
    tabs = ['Home', 'Web Scraping', 'Network Implementation', 'Network Analysis', 'Heatmap', '3D Modelling', 'Bonus part']
    tab_frames = {}
    tab_backgrounds = {
    ...
    for tab in tabs:
        ...
        if tab != 'Home':
            set_tab_background(tab_frames[tab], tab_backgrounds.get(tab, 'home.jpg'))
        if tab == 'Web Scraping':
            def perform_scraping():
                ...
                q = 'openflights flights routes'
                search = GoogleSearch({"q": q, "api_key": api_key})
                results = search.get_dict()

                datasets = []
                for result in results['organic_results']:
                    data_name = result['title']
                    data_url = result['link']
                    datasets.append({'name': data_name, 'url': data_url})

                dataset_index = 1
                scrappedlink = datasets[dataset_index]['url']
                r = requests.get(scrappedlink)
            ...
            display_output = scrolledtext.ScrolledText(tab_frames['Web Scraping'], width=20, height=5)
            ...
            analysis_output = scrolledtext.ScrolledText(tab_frames['Network Analysis'], width=20, height=5)
            ...
            set_tab_background(tab_frames[tab], tab_backgrounds.get(tab, 'bonus.png'))

            def show_graphs():
                nonlocal source, destination

                # Combine source and destination into pairs and calculate frequency
                flight_pairs = list(zip(source, destination))
                pair_counts = Counter(flight_pairs)

                # Sort pairs by frequency and select the top N pairs
                N = 10
                most_common_pairs = pair_counts.most_common(N)
                pairs, counts = zip(*most_common_pairs)
                labels = [f"{src}->{dst}" for src, dst in pairs]

                def create_pie_chart():
                    fig1, ax1 = plt.subplots()
                    ax1.pie(counts, labels=labels, autopct='%1.1f%%')
                    ax1.axis('equal')
                    plt.title("Top Flight Frequency Pie Chart")
                    plt.show()

                def create_bar_chart():
                    plt.bar(labels, counts)
                    plt.xlabel('Flight Route (Source -> Destination)')
                    plt.ylabel('Counts')
                    plt.xticks(rotation=90)
                    plt.title("Top Flight Frequency Bar Chart")
                    plt.show()
                ...
                def create_histogram():
                    ...
                    plt.title("Top Flight Frequency Histogram")
                    plt.show()
            ...
            bonus_label = ttk.Label(tab_frames['Bonus part'], text="The bonus part contains pie, bar, histogram graphs of the data")
            bonus_label.pack(pady=10)
            bonus_btn = ttk.Button(tab_frames['Bonus part'], text="Show graphs", command=show_graphs)
            bonus_btn.pack(pady=10)
